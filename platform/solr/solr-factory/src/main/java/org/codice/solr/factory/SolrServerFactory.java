...
public final class SolrServerFactory {
        ...
        HttpRequestRetryHandler solrRetryHandler = new HttpRequestRetryHandler() {
            ...
            public boolean retryRequest(IOException e, int retryCount, HttpContext httpContext) {
                ...
                try {
                    ...
                    logger.debug("Connection failed, entering grace period for " + waitTime
                            + "seconds.");
                    wait(waitTime);
    ...
    private static long getWaitTime(int retryCount) {
        return (long) Math.pow(2, Math.min(retryCount, MAX_RETRY_COUNT)) * 50;
    }
        ...
        public SolrServer call() throws Exception {
            while (true) {
                try {
                    ...
                    Thread.sleep(getWaitTime(retryCount));
                }
            }
        }
    }
