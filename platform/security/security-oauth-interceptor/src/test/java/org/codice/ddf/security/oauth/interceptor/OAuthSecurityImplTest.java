...
import static org.codice.ddf.security.oauth.interceptor.OAuthSecurity.OAUTH;
import static org.codice.ddf.security.oauth.interceptor.OAuthSecurityImpl.GSON;
...
public class OAuthSecurityImplTest {
  ...
  @Test
  public void testSettingSystemTokensOnClient() {
  ...
  @Test
  public void testSettingSubjectOnClient() throws URISyntaxException {
    Client client = mock(Client.class);
    ...
    TokenInformation.TokenEntry tokenEntry =
        new TokenInformationImpl.TokenEntryImpl(accessToken, "refresh_token", "");
    when(tokenStorage.read(USERNAME, "CSW")).thenReturn(tokenEntry);

    oauthSecurity.setUserTokenOnClient(client, subject, "CSW");
    verify(client, times(1)).header(OAUTH, "Bearer " + accessToken);
  }

  @Test
  public void testSettingSubjectOnClientNoTokens() throws URISyntaxException {
    Client client = mock(Client.class);
    URI uri = new URI("https://localhost:8993/search/catalog");
    when(client.getCurrentURI()).thenReturn(uri);
    Subject subject = getSubject();

    when(tokenStorage.read(USERNAME)).thenReturn(null);
